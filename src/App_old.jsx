...
export default function CorrelationAndRegressionVisualizer() {
  ...
  const [sampleSize, setSampleSize] = useState(200); // Changed default to 200
  ...
  // Get mean Y for horizontal line when not fit
  const meanY = useMemo(() => {
    return scatterData.reduce((acc, d) => acc + d.y, 0) / scatterData.length;
  }, [scatterData]);

  // Decide which slope/intercept to display
  const displayedSlope = fitLine ? slope : 0;
  const displayedIntercept = fitLine ? intercept : meanY; // Changed to use meanY when not fit
        ...
        <CardContent className="flex flex-col items-center">
          ...
          <div className="flex justify-center items-center space-x-4 mb-4">
            <Button
              ...
              variant="default"
              className="px-4 py-2 bg-[#3b82f6] hover:bg-[#2563eb] text-white rounded-2xl shadow"
            >
          ...
          >
            <div className="w-full h-96 overflow-auto flex justify-center">
